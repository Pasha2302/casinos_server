...
function calculateValue(value1, value2, resultValue, maxBet) {
    ...
    resultValue.value = (Number(value1) * Number(value2) / Number(maxBet)).toFixed(2);
}
...
const defineActiveField = () => {
    ...
    const bonusPlusDeposit_2 = document.getElementById('id_wagering_bonus_plus_deposit-0-bonus_plus_deposit');
    const bonusPlusDepositForm = document.getElementsByClassName('form-row field-calculation_bonus_deposit')[0];
    let bonusPlusDepositValue = document.getElementById('id_calculation_bonus_deposit');

    const bonusOnly_1 = document.getElementById('id_bonus_only');
    const bonusOnly_2 = document.getElementById('id_wagering_bonus_plus_deposit-0-bonus_only');
    const bonusOnlyForm = document.getElementsByClassName('form-row field-calculation_bonus_only')[0];
    ...
    function handleInput() {
        if (bonusPlusDeposit_1.value !== '' || bonusPlusDeposit_2.value !== '') {
            ...
            calculateValue(bonusPlusDeposit_1.value, bonusPlusDeposit_2.value, bonusPlusDepositValue, maxBetValue.value);
            ...
            calculateValue(bonusOnly_1.value, bonusOnly_2.value, bonusOnlyValue, maxBetValue.value);
